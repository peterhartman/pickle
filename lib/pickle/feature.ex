    ...
    |> Enum.map(fn describe_block ->
      scenario =
        Map.put(
          ...
          :tags,
          Pickle.global_tags() ++ List.wrap(Module.get_attribute(env.module, :moduletag)) ++ describe_block.scenario.tags
        )
      ...
      quote bind_quoted: [
        ...
        describe describe_block.description do
          setup context do
            ...
            {:ok,
             Map.merge(
               Pickle.Feature.Helpers.fetch_state(unquote(describe_block.description), __MODULE__),
               context || %{}
             )}
          end
          ...
          def unquote(name)(exunit_state) do
            Pickle.Feature.Helpers.start_state(unquote(describe_block.description), __MODULE__, exunit_state)
